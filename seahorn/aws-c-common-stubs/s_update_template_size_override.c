...
/**
 * Function to find the smallest result that is power of 2 >= n. Returns
 * AWS_OP_ERR if this cannot be done without overflow
 */
AWS_STATIC_IMPL int crab_round_up_to_power_of_two(size_t n, size_t *result) {
  if (n == 0) {
    *result = 1;
...
int s_update_template_size(struct hash_table_state *template,
  ...
  size_t size;
  if (crab_round_up_to_power_of_two(min_size, &size)) {
    return AWS_OP_ERR;
  }
