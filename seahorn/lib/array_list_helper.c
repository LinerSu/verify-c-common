...
void initialize_bounded_array_list(struct aws_array_list *const list) {
  list->current_size = nd_size_t();
  list->item_size = nd_size_t();
  CRAB_ASSUME(list->item_size > 0);
  list->length = nd_size_t();
  CRAB_ASSUME(list->length > 0);
  CRAB_ASSUME(aws_array_list_is_bounded(list, sea_max_array_list_len(),
                                        sea_max_array_list_item_size()));
  CRAB_ASSUME(aws_mul_size_checked(list->length, list->item_size,
                                   &list->current_size) == AWS_OP_SUCCESS);
  list->data = can_fail_malloc_havoc(list->current_size);
  list->alloc = sea_allocator();
...
bool aws_array_list_is_bounded(const struct aws_array_list *const list,
  ...
  bool item_size_is_bounded =
      list->item_size == max_item_size; // Avoid non-linear properties
#else
